...
static inline void GetBinds(int iParent, CTFPlayer* pLocal, CTFWeaponBase* pWeapon, std::vector<Bind_t>& vBinds, bool bManage = true)
{
	...
	for (int i = 0; i < vBinds.size(); i++)
	{
		...
		if (bManage)
		{
			...
			{
				// True when playing the specified class AND the key is held simultaneously
				const int iClass = pLocal ? pLocal->m_iClass() : TF_CLASS_UNDEFINED;
				bool bClassMatch = false;
				switch (tBind.m_iExtraInt2)
				{
				case BindEnum::ClassEnum::Scout: bClassMatch = iClass == TF_CLASS_SCOUT; break;
				case BindEnum::ClassEnum::Soldier: bClassMatch = iClass == TF_CLASS_SOLDIER; break;
				case BindEnum::ClassEnum::Pyro: bClassMatch = iClass == TF_CLASS_PYRO; break;
				case BindEnum::ClassEnum::Demoman: bClassMatch = iClass == TF_CLASS_DEMOMAN; break;
				case BindEnum::ClassEnum::Heavy: bClassMatch = iClass == TF_CLASS_HEAVY; break;
				case BindEnum::ClassEnum::Engineer: bClassMatch = iClass == TF_CLASS_ENGINEER; break;
				case BindEnum::ClassEnum::Medic: bClassMatch = iClass == TF_CLASS_MEDIC; break;
				case BindEnum::ClassEnum::Sniper: bClassMatch = iClass == TF_CLASS_SNIPER; break;
				case BindEnum::ClassEnum::Spy: bClassMatch = iClass == TF_CLASS_SPY; break;
				}
				bool bKeyDown = tBind.m_iExtraKey ? U::KeyHandler.Down(tBind.m_iExtraKey, false, &tBind.m_tKeyStorage) : true;
				tBind.m_bActive = bClassMatch && bKeyDown;
				if (tBind.m_bNot)
					tBind.m_bActive = !tBind.m_bActive;
				break;
			}
			...
			{
				// True when local player HP is below configured threshold
				tBind.m_bActive = pLocal ? pLocal->m_iHealth() < tBind.m_iExtraInt : false;
				if (tBind.m_bNot)
					tBind.m_bActive = !tBind.m_bActive;
				break;
			}
			...
			{
				// True when number of real players on server equals configured count
				int iCount = 0;
				if (auto pResource = H::Entities.GetResource())
				{
					for (int n = 1; n <= I::EngineClient->GetMaxClients(); n++)
				...
				tBind.m_bActive = iCount == tBind.m_iExtraInt;
				if (tBind.m_bNot)
					tBind.m_bActive = !tBind.m_bActive;
				break;
			}
			...
			{
				// True when holding weapon with specific item definition index
				tBind.m_bActive = pWeapon ? pWeapon->m_iItemDefinitionIndex() == tBind.m_iExtraWeaponID : false;
				if (tBind.m_bNot)
					tBind.m_bActive = !tBind.m_bActive;
				break;
			}
			...
			{
				// True when player is in a charge state (Demoman shield, Medigun deploy, Sniper Classic)
				bool bCharging = false;
				if (pLocal && pWeapon)
				{
					bCharging = bCharging || pLocal->InCond(TF_COND_SHIELD_CHARGE);
					if (!bCharging && pWeapon->m_flChargeLevel() > 0.f)
					{
						auto pMedi = pWeapon->As<CWeaponMedigun>();
						if (pMedi && pMedi->m_bChargeRelease())
							bCharging = true;
					}
					if (!bCharging)
						bCharging = pWeapon->m_bCharging();
				}
				tBind.m_bActive = bCharging;
				if (tBind.m_bNot)
					tBind.m_bActive = !tBind.m_bActive;
				break;
			}
			...
			{
				// True when another named bool var is currently true
				bool bFound = false;
				for (auto pBase : G::Vars)
				{
					if (pBase->m_iType != typeid(bool).hash_code())
						continue;
					if (!pBase->m_vNames.empty() && (pBase->m_vNames.front() == tBind.m_sOptionName || pBase->Name() == tBind.m_sOptionName))
					{
						tBind.m_bActive = pBase->As<bool>()->Value;
						bFound = true;
						break;
					}
				}
				if (!bFound)
					tBind.m_bActive = false;
				if (tBind.m_bNot)
					tBind.m_bActive = !tBind.m_bActive;
				break;
			}
			case BindEnum::Ticks:
			{
				// True every N ticks (where N is m_iExtraInt)
				if (tBind.m_iExtraInt > 0)
					tBind.m_bActive = (I::GlobalVars->tickcount % tBind.m_iExtraInt) == 0;
				else
					tBind.m_bActive = false;
				if (tBind.m_bNot)
					tBind.m_bActive = !tBind.m_bActive;
			...
			{
				// True when in Casual or Private server
				bool bCasual = false, bPrivate = false;
				if (auto pGameRules = I::TFGameRules)
				{
					int iGroup = pGameRules->GetCurrentMatchGroup();
					bCasual = (iGroup >= k_eTFMatchGroup_Casual_First && iGroup <= k_eTFMatchGroup_Casual_Last);
					bPrivate = (iGroup == k_eTFMatchGroup_Invalid);
				}
				switch (tBind.m_iInfo)
				{
				case BindEnum::GameModeEnum::Casual: tBind.m_bActive = bCasual; break;
				case BindEnum::GameModeEnum::Private: tBind.m_bActive = bPrivate; break;
				}
				if (tBind.m_bNot)
					tBind.m_bActive = !tBind.m_bActive;
				break;
